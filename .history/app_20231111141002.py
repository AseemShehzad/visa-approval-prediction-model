...
outputs = gr.Textbox(label="Probability of getting a visa")
...
with gr.Blocks() as interface:
    with gr.Row():
        ...
        with gr.Column():
            ...
            visa_attempt = visa_attempt
    with gr.Row():
        click_button = click_button
    with gr.Row():
        pred_prob = pred_prob
        
        click_button.click(fn=predict_prob, inputs=inputs, outputs=outputs)

    
# Divide inputs into two side-by-side rows
row1 = gr.Row([university_name, sponsor, relatives])
row2 = gr.Row([program, scholarship, degree_level, visa_attempt])
