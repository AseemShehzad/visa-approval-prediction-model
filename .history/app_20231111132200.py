...
def load_model_files():
    """ Load the necessary model files such as encoder, scaler, and model """
...
def preprocess_input(university_name, sponsor, relatives, program, scholarship, degree_level, visa_attempt, encoder, red_list):
...
def predict_prob(university_name, sponsor, relatives, program, scholarship, degree_level, visa_attempt):
    ...
    print(input_df)
    ### convet the input to a float tensor
    input_df = tf.cast(input_df, tf.float32)    
    predictions = model.signatures['serving_default'](input_df)
    print(predictions)
    return predictions['dense_19'].numpy()[0][0]
