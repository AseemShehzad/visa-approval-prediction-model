...
def process_inputs(university_name, sponsor, relatives, program, scholarship, degree_level, visa_attempt):
    ...
    encoded_cols = encoder.transform(input_df[cat_columns])
    input_df = pd.concat([input_df, pd.DataFrame(encoded_cols)], columns=encoder.get_feature_names_out(cat_columns))], axis=1)
...
def predict_prob(university_name, sponsor, relatives, program, scholarship, degree_level, visa_attempt):
    ...
    print(predictions)
    return predictions['dense_15'].numpy()[0][0]
