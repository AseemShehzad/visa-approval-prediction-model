...
def process_inputs(university_name, sponsor, relatives, program, scholarship, degree_level, visa_attempt):
    ...
    cat_columns = ['Program', 'Scholarship', 'Sponsor', 'Relative(s)']
...
def predict_prob(university_name, sponsor, relatives, program, scholarship, degree_level, visa_attempt):
    ...
    predictions = model.signatures['serving_default'](dense_input=input_df)
    print(predictions)
    return predictions['dense_2'][0][0]
...
# Relatives
relatives = gr.Dropdown(["No", "Sibling(s)", "Relatives", "Parents"], label="Relatives")
