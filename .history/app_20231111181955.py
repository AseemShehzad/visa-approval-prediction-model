...
with gr.Blocks() as interface:
    # Assuming these are properly initialized Gradio components
    university_name = gr.inputs.Text()
    sponsor = gr.inputs.Text()
    relatives = gr.inputs.Number()
    program = gr.inputs.Text()
    scholarship = gr.inputs.Checkbox()
    degree_level = gr.inputs.Radio(["Bachelor's", "Master's", "PhD", "Other"])
    visa_attempt = gr.inputs.Number()

    click_button = gr.Button("Predict")

    pred_prob = gr.outputs.Textbox()

    click_button.click(fn=predict_prob, 
                       inputs=[university_name, sponsor, relatives, program, scholarship, degree_level, visa_attempt], 
                       outputs=pred_prob)
