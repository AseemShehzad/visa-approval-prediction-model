...
def load_model_files():
    ...
    with open('inference_files/red_list.pkl', 'rb') as file:
...
def process_inputs(university_name, sponsor, relatives, program, scholarship, degree_level, visa_attempt, loader):

    # Load the model files
    encoder, red_list, model = load_model_files()
...
### Inferencing
def predict_prob(university_name, sponsor, relatives, program, scholarship, degree_level, visa_attempt, model, encoder, red_list):
    ...
    print(predictions)
    return np.round(predictions['dense_19'].numpy()[0][0] * 100, 3)
