...
# Function to predict the probability (placeholder function)
def predict_prob(university_name, sponsor, relatives, program, scholarship, degree_level, visa_attempt):
    ...
    return "Probability of getting a visa"
...
# Output
pred_prob = gr.Textbox(label="Probability of getting a visa", readonly=True)

# Divide inputs into two side-by-side rows
row1 = gr.Group([university_name, sponsor, relatives], layout="horizontal")
row2 = gr.Group([program, scholarship, degree_level, visa_attempt], layout="horizontal")
...
interface = gr.Interface(
    ...
    inputs=[row1, row2],
    outputs=pred_prob,
    title="Visa Probability Prediction",
    description="Fill in the information below to predict the probability of getting a visa.",
    theme="default", # Choose a theme that matches the desired aesthetic
    layout="vertical"
)
